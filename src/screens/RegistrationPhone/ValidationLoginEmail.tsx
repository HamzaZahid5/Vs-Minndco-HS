...
const MakePopupContent = (navigation: StackNavigationProp<RootStackParamList, keyof RootStackParamList>) => {
  const PopupContent = ({ close }: { close: () => Promise<void> }) => {
    return (
      <>
        <Row gutter={10}>
          <Subheading>{translate('screens.ValidationLoginEmail.titlePopup')}</Subheading>
        </Row>
        <Row grow justifyContentOnGrow="flex-start" gutter={10}>
          <Paragraph size="medium" weight="normal" textAlign="left">
            {translate('screens.ValidationLoginEmail.messagePopup')}
          </Paragraph>
        </Row>
        <Row gutter={10} grow justifyContentOnGrow="flex-end">
          ...
          >
            {translate('screens.ValidationLoginEmail.support', {
          ...
          >
            {translate('screens.ValidationLoginEmail.retry', {
              defaultValue: 'Retry',
            })}
          </Button>
        </Row>
...
export const ValidationLoginEmail = ({ route }: DefaultScreenRouteType<'ValidationLoginEmail'>) => {
  // UTILS
  const theme = useRobTheme()
  ...
  const handleSubmit = async () => {
    setIsLoading(true)
    try {
      const { data: result } = await functions().httpsCallable('loginWithEmail')({
        email,
      ...
      <BasicScreen>
        ...
        <Row gutter={50} justifyContentOnGrow="center">
          <Paragraph size="small" weight="normal">
            {translate('screens.ValidationLoginEmail.textToVerifyCode', {
      ...
      <View style={{ position: 'absolute', bottom: 30, left: 25, right: 25, flexDirection: 'row' }}>
        <View style={{ flex: 1 }}>
          <Button role="secondary" onPress={() => navigation.goBack()}>
            {translate('screens.ValidationLoginEmail.goBack', {
        ...
        <View style={{ flex: 1 }}>
          <Button role="primary" onPress={handleSubmit} disabled={text.length !== 6}>
            {translate('screens.ValidationLoginEmail.confirm', {
