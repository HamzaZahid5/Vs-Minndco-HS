...
const WelcomeWizard = ({ navigation }) => {
  ...
  function finishWizard(navigateTo) {
    ...
    // const navigate = navigateTo || navigateBack;
  ...
  return (
    <>
      {currentStep === 0 && (
        <HomeLayout
          rowTopStyle={styles.rowTop}
          rowBottomStyle={styles.rowBottom}
        />
      )}
      {currentStep === 1 && (
        <Step1
          ...
          message="Let's take one step at a time together, shall we?"
        />
      )}
      {currentStep === 2 && (
        <Step2
      ...
      {currentStep === 4 && (
        <Step5
          ...
          message={"Your daily training is the most important part of this program. I suggest you practice one activity per day, every day!\nA playlist of VR and audiovisual short exercises to get you ready to manage the stress episodes!"}
        />
      )}
      {currentStep === 5 && (
        <Step6
